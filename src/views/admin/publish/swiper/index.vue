    ...
    >
      ...
      <el-table-column label="操作" align="center" width="330" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.swiperStatus =='wait'" size="mini" type="success"
          @click="toReviewed(row)" >
            审核
          </el-button>
           <el-button v-if="row.swiperStatus =='published'" size="mini" type="danger"
          @click="xiajia(row)" >
            下架
          </el-button>
          <el-button v-if="row.swiperStatus =='reviewed'" size="mini" type="success"
          @click="toPublished(row)" >
            上架
          </el-button>
          <el-button v-if="row.swiperStatus =='wait'" size="mini" type="danger"
          @click="toDraft(row)" >
            打回
          </el-button>
        </template>
...
<script>
import { getSwiperList,updateSwiper} from '@/api/admin/admin'
...
export default {
  ...
  methods: {
    ...
    toReviewed(swiper){
      swiper.swiperStatus = 'reviewed'
      updateVideo(swiper).then(response =>{
           this.$notify({
          message: '审核成功',
          type: 'success'
        });
